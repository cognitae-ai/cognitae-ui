...
import { COGNITAE_PROMPT } from '../../lib/prompt';
import { CDB, CT, genId } from '../../lib/db';
import { C, FONT, sans, serif } from '../../lib/constants';
import { Link } from 'react-router-dom';

export default function CognitaeShell() {
    ...
    useEffect(() => {
        CT.loadSetting("gemini_key").then(k => {
            if (k) setApiKey(k);
            else console.log("No Gemini API key found in CognitaeDB");
        });
    }, []);
    ...
    const saveKey = async (e) => {
        ...
        setApiKey(val);
        await CT.saveSetting("gemini_key", val);
    };
    ...
    const handleSend = async () => {
        ...
        try {
            const reply = await askGemini(apiKey, COGNITAE_PROMPT, msgs, newM[newM.length - 1].content);
    ...
    return (
        <div style={{ display: 'flex', height: '100vh', background: C.bg, color: C.text, fontFamily: sans }}>
            <div style={{ width: 280, borderRight: `1px solid ${C.border}`, padding: 24, display: 'flex', flexDirection: 'column' }}>
                ...
                <h2 style={{ fontFamily: serif, color: C.accent, fontWeight: 400, fontSize: 24, letterSpacing: '0.05em', marginBottom: 32 }}>Cognitae</h2>
                ...
                <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 11, color: C.dim, lineHeight: 1.6 }}>System Instruction loaded: "You are Cognitae. Quiet Intensity..."</div>
                </div>
            </div>
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '0 48px' }}>
                <div style={{ flex: 1, overflowY: 'auto', padding: '48px 0' }}>
                    ...
                    {thinking && (
                        <div style={{ maxWidth: 640, margin: "0 auto", color: C.dim }}>Synthesizing...</div>
                    )}
                </div>
